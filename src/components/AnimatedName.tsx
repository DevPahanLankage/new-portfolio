...
vec4 draw(vec2 uv, vec2 p, float t, float scale) {
  ...
  c *= a1;
  c += cellsColor(p * scale, scale, t) * 2.0;
  return c;
}
...
void main() {
  ...
  float t = clamp((enterTime - 0.1) * 1.5, 0., 1.);
  
  vec4 color = vec4(0);
  for (float i = 0.; i < LAYERS; i++) {
    float s = cos(i) * 7.3 + 10.;
    color += draw(uv, p, t, abs(s));
  }
  color /= LAYERS;
  
  gl_FragColor = color * smoothstep(0., 0.01, t);
}
`
...
export default function AnimatedName() {
  ...
  return (
    ...
    >
      <div 
        ref={elementRef} 
        className="relative inline-block text-7xl font-medium tracking-tight bg-gradient-to-r from-amber-400 via-orange-500 to-rose-400 text-transparent bg-clip-text leading-normal pb-2"
      >
        Pahan Lankage
      </div>
      <motion.span
        className="absolute -bottom-2 left-0 w-[72%] h-1 bg-yinmn-blue rounded-full"
